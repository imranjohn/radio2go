<template>
  <div>
    ...
    <div class="mb-6 flex justify-between items-center">
      ...
      <inertia-link class="btn-indigo" :href="route('brand-stations.create')">
        <span>Create</span>
        <span class="hidden md:inline">Brand Stations</span>
      </inertia-link>
    </div>
    <div class="bg-white rounded-md shadow overflow-x-auto">
      <table class="w-full whitespace-nowrap">
        ...
        <tr v-for="station in stations.data" :key="station.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
          <td class="border-t">
            <inertia-link class="px-6 py-4 flex items-center focus:text-indigo-500" :href="route('brand-stations.edit', station.id)">
              {{ station.name }}
        ...
        <tr v-if="stations.data.length === 0">
          <td class="border-t px-6 py-4" colspan="4">No brand stations found.</td>
        </tr>
      </table>
    </div>
  ...
  watch: {
    form: {
      ...
      handler: throttle(function() {
        this.$inertia.get(this.route('brand-stations.index'), pickBy(this.form), { preserveState: true })
      }, 150),
    },
  },
  methods: {
    ...
    destroy(id) {
      if (confirm('Are you sure you want to delete this station?')) {
        this.$inertia.delete(this.route('brand-stations.destroy', id))
      }
    },
  },
}
